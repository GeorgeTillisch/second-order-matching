...
describe("The MatchingChallenge class (solving)", () => {
  test("should correctly solve example challenges from the paper", () => {
    ...
    // Example 2
    constraints = newConstraints(["multiply(plus(_X,_Y),minus(_X,_Y))", "multiply(plus(3,k),minus(3,p))"]);
    mc = newMC(constraints);
    sols = mc.getSolutions();
    ...
    mc = newMC(constraints);
    sols = mc.getSolutions();
    expect(checkSolutions(sols, newSolutions([["_P", lambdaString("v.neq(0,v)")]]))).toBe(true);
  });
  ...
  test("should correctly solve small challenges", () => {
    ...
    ////////// Test 1 //////////
    constraints = newConstraints(["_P_of__x", "b(2)"], ["_P_of__y", "b(3)"]);
    mc = newMC(constraints);
    sols = mc.getSolutions();
    expect(
      checkSolutions(
    ...
    ////////// Test 2 //////////
    constraints = newConstraints(["_P_of__x", "equ.als(pl.us(2,3),5)"], ["_P_of__y", "equ.als(5,5)"]);
    mc = newMC(constraints);
    sols = mc.getSolutions();
    expect(
      checkSolutions(
    ...
    ////////// Test 3 //////////
    constraints = newConstraints(["_P_of__x", "A(1,2,3)"], ["_P_of__y", "A(2,1,3)"]);
    mc = newMC(constraints);
    sols = mc.getSolutions();
    expect(
      checkSolutions(
    ...
    ////////// Test 4 //////////
    constraints = newConstraints(["_P_of__x", "A(1,2,3)"], ["_P_of__x", "A(2,1,3)"]);
    mc = newMC(constraints);
    sols = mc.getSolutions();
    expect(sols.length).toBe(0);

    ////////// Test 5 //////////
    constraints = newConstraints(["_P_of__x", "f(1,2)"], ["_P_of__y", "f(1,2)"]);
    mc = newMC(constraints);
    sols = mc.getSolutions();
    expect(
      checkSolutions(
    ...
    ////////// Test 6 //////////
    constraints = newConstraints(["_P_of__x", "g(k,e(2))"]);
    mc = newMC(constraints);
    sols = mc.getSolutions();
    expect(
      checkSolutions(
    ...
    ////////// Test 7 //////////
    constraints = newConstraints(["_P_of__x", "f(a,a)"]);
    mc = newMC(constraints);
    sols = mc.getSolutions();
    expect(
      checkSolutions(
    ...
    ////////// Test 8 //////////
    constraints = newConstraints(["_P_of__x", "f(a,a)"], ["_x", "b"]);
    mc = newMC(constraints);
    sols = mc.getSolutions();
    expect(
      checkSolutions(
    ...
    ////////// Test 9 //////////
    constraints = newConstraints(["_P_of__x", "f(a,a)"], ["_x", "f"]);
    mc = newMC(constraints);
    sols = mc.getSolutions();
    expect(
      checkSolutions(
    ...
    ////////// Test 10 //////////
    constraints = newConstraints(["_P_of__a", "eq(3,3)"], ["_Q_of__b", "gt(5,4)"]);
    mc = newMC(constraints);
    sols = mc.getSolutions();
    expect(
      checkSolutions(
  ...
  test("should correctly solve challenges involving the equality elimination rule", () => {
    ...
    ////////// Test 11 //////////
    constraints = newConstraints(["eq(_a,_b)", "eq(t,1)"], ["_P_of__a", "gt(t,0)"], ["_P_of__b", "gt(1,0)"]);
    mc = newMC(constraints);
    sols = mc.getSolutions();
    expect(
      checkSolutions(
    ...
    ////////// Test 12 //////////
    constraints = newConstraints(["eq(_a,_b)", "eq(t,1)"], ["_P_of__a", "gt(1,0)"], ["_P_of__b", "gt(t,0)"]);
    mc = newMC(constraints);
    sols = mc.getSolutions();
    expect(sols.length).toBe(0);
  ...
  test("should correctly solve challenges involving the universal elimination rule", () => {
    ...
    ////////// Test 22 //////////
    constraints = newConstraints(["for.all[_x,_P_of__x]", "for.all[x,gte(x,0)]"], ["_P_of__t", "gte(7,0)"]);
    mc = newMC(constraints);
    sols = mc.getSolutions();
    expect(
      checkSolutions(
    ...
    ////////// Test 23 //////////
    constraints = newConstraints(["for.all[_x,_P_of__x]", "for.all[x,gte(x,0)]"], ["_P_of__t", "gte(7,7)"]);
    mc = newMC(constraints);
    sols = mc.getSolutions();
    expect(sols.length).toBe(0);

    ////////// Test 24 //////////
    constraints = newConstraints(["for.all[_x,_P_of__x]", "for.all[x,Q]"], ["_P_of__t", "Q"]);
    mc = newMC(constraints);
    sols = mc.getSolutions();
    expect(
      checkSolutions(
    ...
    ////////// Test 25 //////////
    constraints = newConstraints(["for.all[_x,_P_of__x]", "for.all[s,eq(sq(s),s)]"], ["_P_of__t", "eq(sq(1),1)"]);
    mc = newMC(constraints);
    sols = mc.getSolutions();
    expect(
      checkSolutions(
    ...
    ////////// Test 26 //////////
    constraints = newConstraints(["for.all[_x,_P_of__x]", "for.all[x,R(x,y)]"], ["_P_of__t", "R(x,3)"]);
    mc = newMC(constraints);
    sols = mc.getSolutions();
    expect(sols.length).toBe(0);

    ////////// Test 27 //////////
    constraints = newConstraints(["for.all[_x,_P_of__x]", "for.all[x,R(x,y)]"], ["_P_of__t", "R(3,y)"]);
    mc = newMC(constraints);
    sols = mc.getSolutions();
    expect(
      checkSolutions(
    ...
    ////////// Test 28 //////////
    constraints = newConstraints(["for.all[_x,_P_of__x]", "for.all[x,R(x,x)]"], ["_P_of__t", "R(3,3)"]);
    mc = newMC(constraints);
    sols = mc.getSolutions();
    expect(
      checkSolutions(
    ...
    ////////// Test 29 //////////
    constraints = newConstraints(["for.all[_x,_P_of__x]", "for.all[x,R(x,x)]"], ["_P_of__t", "R(3,x)"]);
    mc = newMC(constraints);
    sols = mc.getSolutions();
    expect(sols.length).toBe(0);

    ////////// Test 30 //////////
    constraints = newConstraints(["for.all[_x,_P_of__x]", "for.all[x,R(x,x)]"], ["_P_of__t", "R(x,3)"]);
    mc = newMC(constraints);
    sols = mc.getSolutions();
    expect(sols.length).toBe(0);

    ////////// Test 31 //////////
    constraints = newConstraints(["for.all[_x,_P_of__x]", "for.all[x,R(x,x)]"], ["_P_of__t", "R(x,x)"]);
    mc = newMC(constraints);
    sols = mc.getSolutions();
    expect(
      checkSolutions(
    ...
    ////////// Test 33 //////////
    constraints = newConstraints(["for.all[_x,_P_of__x]", "for.all[x,eq(x,x)]"], ["_P_of__t", "eq(iff(P,Q),iff(P,Q))"]);
    mc = newMC(constraints);
    sols = mc.getSolutions();
    expect(
      checkSolutions(
  ...
  test("should correctly solve challenges involving the universal introduction rule", () => {
    ...
    mc = newMC(constraints);
    sols = mc.getSolutions();
    expect(checkSolutions(sols, newSolutions([["_P", lambdaString("v.r(v,v)")]]))).toBe(true);
  ...
  test("should correctly solve challenges involving the existential introduction rule", () => {
    ...
    ////////// Test 50 //////////
    constraints = newConstraints(["_P_of__t", "gt(1,0)"], ["exi.sts[_x,_P_of__x]", "exi.sts[x,gt(x,0)]"]);
    mc = newMC(constraints);
    sols = mc.getSolutions();
    expect(
      checkSolutions(
    ...
    ////////// Test 52 //////////
    constraints = newConstraints(["_P_of__t", "lt(pow(t,x),5)"], ["exi.sts[_x,_P_of__x]", "exi.sts[x,lt(pow(x,x),5)]"]);
    mc = newMC(constraints);
    sols = mc.getSolutions();
    expect(sols.length).toBe(0);

    ////////// Test 53 //////////
    constraints = newConstraints(["_P_of__t", "neq(x,t)"], ["exi.sts[_x,_P_of__x]", "exi.sts[y,neq(y,t)]"]);
    mc = newMC(constraints);
    sols = mc.getSolutions();
    expect(
      checkSolutions(
    ...
    ////////// Test 54 //////////
    constraints = newConstraints(["_P_of__t", "neq(x,t)"], ["exi.sts[_x,_P_of__x]", "exi.sts[x,neq(x,x)]"]);
    mc = newMC(constraints);
    sols = mc.getSolutions();
    expect(sols.length).toBe(0);
  ...
  test("should correctly solve challenges involving induction on the natural numbers", () => {
    ...
    constraints = newConstraints(
      ["_P_of_0", "eq(7,5)"],
      ["for.all[_k,implies(_P_of__k,SecondOrderMatching.gEFA(_P,plus(_k,1)))]", "for.all[n,implies(eq(7,5),eq(7,5))]"],
