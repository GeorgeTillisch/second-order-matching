...
export function quick(string) {
  ...
  var variables = tree.descendantsSatisfying((x) => {
    return x.type == "v";
  });
  ...
  var symbols = tree.descendantsSatisfying((x) => {
    return x.type == "sy";
  });
...
export const newConstraintObject = (pattern_string, expression_string) => {
  return new M.Constraint(quick(pattern_string), quick(expression_string));
};
...
export const newConstraints = (...string_pairs) => {
  ...
  for (let i = 0; i < string_pairs.length; i++) {
    ...
    constraints.push(string_pair.map((s) => quick(s)));
  }
  return constraints;
};
