...
export class OMNode {
  ...
  static checkJSON(object) {
    ...
    const identRE = /^[:A-Za-z_\u0374-\u03FF][:A-Za-z_\u0374-\u03FF.0-9-]*$/;
  ...
  static decode(json) {
    ...
    let fixByteArrays = function (node) {
      ...
      for (let c of node.c != null ? node.c : []) {
        // children, if any
        fixByteArrays(c);
      }
  ...
  get children() {
    return (this.tree.c != null ? this.tree.c : []).map(
      (child) => new OMNode(child)
    );
  }
  get variables() {
    if (this.tree.t === "bi") {
  ...
  static symbol(name, cd, uri) {
    return OMNode.decode(
      uri != null ? { t: "sy", n: name, cd, uri } : { t: "sy", n: name, cd }
    );
  }
  ...
  simpleEncode() {
    var recur = function (tree) {
      switch (tree != null ? tree.t : undefined) {
        ...
        default:
          return `Error: Invalid OpenMath type ${
            tree != null ? tree.t : undefined
          }`;
      }
    };
  ...
  setAttribute(keySymbol, newValue) {
    ...
    newValue.remove();
    (this.tree.a != null ? this.tree.a : (this.tree.a = {}))[
      keySymbol.encode()
    ] = newValue.tree;
  ...
  freeVariables() {
    switch (this.type) {
      ...
      case "bi":
        ...
        return this.body
          .freeVariables()
          .filter((varname) => !boundByThis.includes(varname));
      default:
        return [];
    }
  }
    ...
    case "a":
      switch (this.children[0].simpleEncode()) {
        ...
        case "arith1.power":
          ...
          break;
        case "arith1.root":
          result = call((a, b) => Math.pow(b, 1 / a), 1, 2);
          break;
        case "arith1.abs":
        ...
        case "relation1.approx":
          var tmp = call((a, b) => Math.abs(a - b) < 0.01, 1, 2);
        ...
        case "transc1.tan":
          ...
          break;
        case "transc1.cot":
          result = call((a) => 1 / Math.tan(a), 1);
          break;
        case "transc1.sec":
          result = call((a) => 1 / Math.cos(a), 1);
          break;
        case "transc1.csc":
          result = call((a) => 1 / Math.sin(a), 1);
          break;
        case "transc1.arcsin":
        ...
        case "transc1.arctan":
          ...
          break;
        case "transc1.arccot":
          result = call((a) => Math.atan(1 / a), 1);
          break;
        case "transc1.arcsec":
          result = call((a) => Math.acos(1 / a), 1);
          break;
        case "transc1.arccsc":
          result = call((a) => Math.asin(1 / a), 1);
          break;
        case "transc1.ln":
