...
export class MatchingChallenge {
  ...
  clone() {
    ...
    result.solutions = this.solutions === undefined ? undefined : this.solutions.map((sol) => sol.copy());
  ...
  solutionSatisfiesBindingConstraints(solution) {
    return this.challengeList.bindingConstraints.every((binding_constraint) => {
      ...
      const outer = isMetavariable(binding_constraint.outer)
        ? solution.lookup(binding_constraint.outer)
        : binding_constraint.outer;
  ...
  getOneSolution() {
    ...
    if (this.solvable === true && this.solutions !== undefined && this.solutions.length > 0) return this.solutions[0];
  ...
  getSolutions() {
    if (this.solvable === undefined || this.solutions === undefined) {
      // try {
      let solutions = [];
      for (let solution of this.solutionsIterator()) solutions.push(solution);
          ...
          if (current_constraint.pattern.type === "bi" && current_constraint.expression.type === "bi") {
            ...
            for (let i = 0; i < pattern_vars.length; i++) {
              ...
              if (!isMetavariable(variable)) {
                ...
                current_constraint.pattern = alphaConvert(current_constraint.pattern, pattern_vars[i], new_var);
              }
            }
          }
        ...
        case CASES.EFA:
          ...
          for (let i = 2; i < current_constraint.pattern.children.length; i++) {
            ...
            let new_vars = current_constraint.pattern.children
              .slice(2)
              .map(() => temp_mc_B.challengeList.nextNewVariable());
            let proj_sub = new Constraint(head, makeProjectionExpression(new_vars, new_vars[i - 2]));
          ...
          if (expression.type === "a" || expression.type === "bi") {
            ...
            let new_vars = current_constraint.pattern.children
              .slice(2)
              .map(() => temp_mc_C.challengeList.nextNewVariable());
            ...
            let imitation_sub = new Constraint(current_constraint.pattern.children[1], imitation_expr);
